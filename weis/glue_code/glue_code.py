...
class WindPark(om.Group):
    ...
    def setup(self):
        ...
        tune_rosco_ivc.add_output("SetpointIPC_Tilt_k", val=np.zeros(modeling_options['ROSCO']['SetpointIPC_nHarmonics']), desc="Setpoint IPC tilt amplitudes for different harmonics.")
        tune_rosco_ivc.add_output("SetpointIPC_Yaw_k", val=np.zeros(modeling_options['ROSCO']['SetpointIPC_nHarmonics']), desc="Setpoint IPC yaw amplitudes for different harmonics.")
        ...
        if modeling_options["ROSCO"]["flag"]:
            ...
            self.connect('tune_rosco_ivc.SetpointIPC_Tilt_k', 'sse_tune.tune_rosco.SetpointIPC_Tilt_k')
            self.connect('tune_rosco_ivc.SetpointIPC_Yaw_k', 'sse_tune.tune_rosco.SetpointIPC_Yaw_k')
        ...
        if modeling_options["OpenFAST"]["flag"] or modeling_options["OpenFAST_Linear"]["flag"]:
            ...
            if modeling_options["General"]["openfast_configuration"]["model_only"] == False:
                ...
                self.connect("tune_rosco_ivc.SetpointIPC_Tilt_k", "outputs_2_screen_weis.SetpointIPC_Tilt_k")
                self.connect("tune_rosco_ivc.SetpointIPC_Yaw_k", "outputs_2_screen_weis.SetpointIPC_Yaw_k")
                #if modeling_options["ROSCO"]["Flp_Mode"]:
